...
// TODO : change to check if player head is eaten by blob or hits wall
...
void checkIfPlayerHeadHitSnake(const byte playerID, const byte playerIndex) {
  if(players[playerID].isActive == 1 && players[playerIndex].isActive == 1) {
    ...
    for (int snakeBodyIterator = 0; snakeBodyIterator < maxBlobSize; snakeBodyIterator++) {
...
// TODO : player doesn't move continuously. Change logic here.
void movePlayer(const byte playerID) {
  ...
  if (millis() - players[playerID].lastMovingTime >= players[playerID].movingSpeed) {
    ...
    for(byte playerBodyIndex = 0; playerBodyIndex < maxBlobSize - 1; playerBodyIndex++) {
    ...
    for(byte playerBodyIndex = 0; playerBodyIndex < maxBlobSize; playerBodyIndex++) {
...
// TODO : player isn't a snake anymore, display the blob.
